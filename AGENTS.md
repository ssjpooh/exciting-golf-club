...
### ⚠️ 중요한 히스토리 — 볼링 앱에서 골프 앱으로 전환됨
이 프로젝트는 원래 **볼링 스코어 앱**이었고 골프로 용도 변경(repurpose)되었다. 그래서 아래처럼 **볼링 잔재**가 남아 있다. 수정 시 혼동하지 말 것:
- `src/lib/version.ts`의 `RELEASE_NOTES`가 아직 "익사이팅 볼링 클럽", "볼링장", "볼링 점수"를 언급함.
- `src/lib/db.ts`의 `createOrUpdateUser`에 닉네임 기본값 마이그레이션 로직이 `"볼링러"`를 걸러냄.
- 일부 오래된 커밋 메시지가 볼링 맥락.
> 새 릴리스 노트/문구를 쓸 때는 골프 기준으로 정리할 것.
...
5. **볼링 잔재**: `version.ts` 릴리스 노트 문구가 골프와 불일치(§1 참고).
6. **슈퍼 관리자 이메일 목록 불일치**: `db.ts`와 `scores.tsx`의 하드코딩 목록이 다름(§6 참고).
