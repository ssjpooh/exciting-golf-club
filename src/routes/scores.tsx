  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
          ...
          {isSuperAdminEmail && (
            <div className="relative">
              <Button 
                variant="ghost" 
                size="sm" 
                className="p-1 h-8 w-8 hover:bg-slate-100 rounded cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              {isAdminMenuOpen && (
                <>
                  ...
                  />
                  <div className="absolute left-0 mt-1 w-36 bg-white border rounded-md shadow-lg z-20">
                    <button
                      onClick={() => {
      ...
      <main className="p-4 max-w-2xl mx-auto space-y-6">
        ...
        {/* 조회 옵션 바: 입력 창 글자 크기 (라운딩 타입 필터는 통계 그래프 제목 앞으로 이동) */}
        {(() => {
        ...
        {/* ⛳ 9홀/18홀 게임별 파, 버디, 보기, 양파 수치 통계 그래프 영역 (라운딩 타입 필터 포함) */}
        <ScoreStatisticsGraph
          games={displayGames}
          fontSizePreset={fontSizePreset}
          roundTypeFilter={roundTypeFilter}
          onRoundTypeChange={setRoundTypeFilter}
        />
        ...
        <div className="space-y-4">
          {displayGames.map(game => {
            ...
            if (fontSizePreset === "medium") {
              ...
              cardDateText = "text-base sm:text-lg text-slate-500 font-black";
              cardHeaderBadge = "text-sm sm:text-base font-bold bg-slate-100 px-3 py-1 rounded text-slate-700";
              cardNetBadge = "text-sm sm:text-base font-bold text-teal-700 bg-teal-50 px-3 py-1 rounded border border-teal-350";
              cardTitle = "text-base sm:text-lg font-bold flex items-center gap-2";
              cardStatsText = "flex gap-3 mt-4 text-sm sm:text-base font-black";
              cardTableText = "text-sm sm:text-base font-extrabold";
